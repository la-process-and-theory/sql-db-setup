...
db_password <- 'testsql!'
db_name <- 'oudb'
db_host <- 'database-1.cigrgbfkibwt.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
BankWages <- read.csv("BankWages.csv", header = TRUE)
CartelStability <- read.csv("CartelStability.csv", header = TRUE)

dbWriteTable(mydb, "BankWages", BankWages)
dbWriteTable(mydb, "CartelStability", CartelStability)

dbListTables(mydb)

dbReadTable(mydb, 'CartelStability')
dbReadTable(mydb, 'BankWages')
```
...
dbGetQuery(mydb, "SELECT X, quantity FROM CartelStability ORDER BY X DESC LIMIT 20;")
#Read the other table according to a condition of one of the variables.
dbGetQuery(mydb, "SELECT X AS 'Number', ice FROM CartelStability LIMIT 20;") 
```
...
dbGetQuery(mydb, "INSERT INTO CartelStability ( X,price, cartel, quantity) VALUES ('329','0.35', 'yes', '1532 ');")
dbGetQuery(mydb, "SELECT * FROM CartelStability ORDER BY quantity LIMIT 10;")


dbGetQuery(mydb, "DELETE FROM CartelStability WHERE quantity = 1532;")
dbGetQuery(mydb, "DELETE FROM CartelStability WHERE quantity = 0;")
...
dbGetQuery(mydb,"CREATE TABLE BankWages3 (
  X INTEGER, 
  job TEXT,
  education INTEGER,
  gender TEXT,
...
dbListTables(mydb)
dbGetQuery(mydb, "DROP TABLE Details;")
dbGetQuery(mydb, "DROP TABLE BankWages;")
dbGetQuery(mydb, "DROP TABLE BankWages2;")
```
...
dbGetQuery(mydb,"CREATE TABLE BankWages (
  X INTEGER, 
  job TEXT,
  education INTEGER,
  gender TEXT,
...
dbGetQuery(mydb, "DROP TABLE IF EXISTS BankWages;")
...
dbGetQuery(mydb,"INSERT INTO BankWages (X, job,education,gender,minority) VALUES ('1', 'admin','15','female','yes');")
dbListTables(mydb)
dbGetQuery(mydb, "DROP TABLE BankWages;")
```
...
dbGetQuery(mydb, "ALTER TABLE CartelStability ADD constant INTEGER DEFAULT 3 ")
dbGetQuery(mydb, "SELECT * FROM CartelStability LIMIT 10;")
dbGetQuery(mydb, "ALTER TABLE CartelStability DROP COLUMN constant;")
```
...
dbGetQuery(mydb, "SELECT X, quantity FROM CartelStability WHERE X > 5 AND quantity >40000 ORDER BY X DESC;")
```
...
dbGetQuery(mydb,"INSERT INTO CartelStability (X,price,cartel,quantity,ice) VALUES ('1', '0.4','yes','13632','yes');")

dbGetQuery(mydb, "SELECT DISTINCT ice FROM CartelStability;")
dbGetQuery(mydb, "SELECT DISTINCT ice, price FROM CartelStability;")
```
...
dbGetQuery(mydb, "DROP TABLE newTable4;")
dbGetQuery(mydb, "DROP TABLE Table5;")

dbGetQuery(mydb,"CREATE TABLE newTable4 (
  id INTEGER AUTO_INCREMENT PRIMARY KEY, 
  score INTEGER, 
  student TEXT,
  Type TEXT,
...
dbGetQuery(mydb, "INSERT INTO newTable4 VALUES (11, 60, 'Ann','h',45);")
dbGetQuery(mydb, "INSERT INTO newTable4 VALUES (12, 60, 'John','j',78);")
dbGetQuery(mydb, "INSERT INTO newTable4 VALUES (13, 70, 'Peter','t',42);")
dbGetQuery(mydb, "INSERT INTO newTable4 VALUES (14, 80, 'Zang','r',12);")
dbGetQuery(mydb, "INSERT INTO newTable4 VALUES (15, 40, 'Kate','ui',56);")
...
dbGetQuery(mydb, "INSERT INTO Table5 VALUES (12, 5, 'Medium','A',445);")
dbGetQuery(mydb, "INSERT INTO Table5 VALUES (13, 7, 'Basic','A',787);")
dbGetQuery(mydb, "INSERT INTO Table5 VALUES (14, 7, 'Advance','F',42);")
dbGetQuery(mydb, "INSERT INTO Table5 VALUES (15, 2, 'Basic','C',124);")
dbGetQuery(mydb, "INSERT INTO Table5 VALUES (16, 6, 'Medium','B',456);")

dbGetQuery(mydb, "SELECT * FROM newTable4;")
dbGetQuery(mydb, "SELECT * FROM Table5;")

dbGetQuery(mydb,"SELECT T.id AS Table5, n.id AS newTable4 
           FROM Table5 AS T 
           JOIN newTable4 AS n ON T.id = n.id")

dbGetQuery(mydb,"SELECT T.id AS Table5, n.id AS newTable4 
           FROM Table5 AS T
           RIGHT JOIN newTable4 AS n ON n.id = n.id")

dbGetQuery(mydb,"SELECT T.id AS Table5, n.id AS newTable4 
           FROM Table5 AS T 
           LEFT JOIN newTable4 AS n ON T.id = n.id")
