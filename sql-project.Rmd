...
db_password <- 'testsql!'
db_name <- 'oudb'
db_host <- 'database-1.c79ld4ijqfoj.us-east-1.rds.amazonaws.com'
db_port <- 3306
...
library(dplyr)
sample1<-select(studentInfo,"id_student","region","age_band","gender")
sample2<-select(studentInfo,"id_student","studied_credits","final_result")

dbReadTable(mydb, "students_info",sample1)
dbReadTable(mydb, "students_result",sample2)
...
dbGetQuery(mydb, "SELECT id_student, gender, age_band * FROM sample1 ORDER BY id_student LIMIT 20;")
dbGetQuery(mydb, "SELECT COUNT(*) FROM sample2 WHERE final_results='Distinction';")
dbGetQuery(mydb, "SELECT COUNT(*) FROM sample2 WHERE studied_credits > 100 ;")
...
#insert
dbGetQuery(mydb, "INSERT INTO sample1 (id_student, region, age_band, gender) VALUES ('11111', 'Null', '0-35', 'M');")
dbGetQuery(mydb, "SELECT * FROM sample1 ORDER BY id_student;")

#change value
dbGetQuery(mydb, "UPDATE sample2 SET final_results = 'Distinction' WHERE id_student = 11391;")

#delete
dbGetQuery(mydb, "DELETE FROM sample1 WHERE region= 'NULL';")
dbGetQuery(mydb, "SELECT * FROM sample1 ORDER BY id_student;")

dbGetQuery(mydb, "DELETE FROM sample2 WHERE id_students = 11391;")
dbGetQuery(mydb, "SELECT * FROM sample2 ;")
...
dbGetQuery(mydb,"CREATE TABLE sqlsample1 (
  id_student INTEGER, 
  region TEXT,
  age_band INTEGER,
  gender TEXT
  );")
dbGetQuery(mydb,"SELECT * FROM sqlsample1;")
dbGetQuery(mydb, "DROP TABLE sample1;")
...
#Recreat
dbGetQuery(mydb,"CREATE TABLE sqlsample1_recreate (
...
#Test, Display, Delete
dbGetQuery(mydb,"INSERT INTO sqlsample1_recreate (
    id_student, 
    region，
    age_band, 
    gender) 
    VALUES ('28400', 'Scotland','55<=','F');")
...
dbGetQuery(mydb,"INSERT INTO sqlsample1_recreate (
    ...
    age_band, 
    gender) 
    VALUES ('38053', 'Wales','35-55','M');")

dbGetQuery(mydb, "SELECT * FROM sqlsample1_recreate;")
dbGetQuery(mydb, "DROP TABLE sqlsample1_recreate;")
...
dbGetQuery(mydb, "ALTER TABLE sample2 ADD rank INTEGER DEFAULT 3 ")
dbGetQuery(mydb, "SELECT * FROM sample2;")
dbGetQuery(mydb, "ALTER TABLE sample2 DROP COLUMN rank;")
dbListTables(mydb)
...
dbGetQuery(mydb,"INSERT INTO sample3 (
          id_student,
          age,
          name,
          email) VALUES (00001, 25,'Ann','xxx01@columbia.edu');")

dbGetQuery(mydb,"INSERT INTO sample3 (
          id_student,
          age,
          name,
          email) VALUES (00002, 22,'Bob','xxx02@columbia.edu');")
...
dbGetQuery(mydb, "SELECT id_student FROM sample2  WHERE studied_credits > 150 ORDER BY studied_credits DESC;")

dbGetQuery(mydb, "SELECT id_student FROM sample2  WHERE final_result = fail;")
...
dbGetQuery(mydb, "INSERT INTO smaple2 Values ('11391','240','Pass');")
dbGetQuery(mydb, "SELECT DISTINCT id_student, studied_credits, final_result FROM sample2;")
...
dbGetQuery(mydb, "CREATE TABLE sample1 (
           id_student INTEGER, 
           region TEXT,
           age_band INTEGER,
           gender TEXT);")

dbGetQuery(mydb, "CREATE TABLE sample2 (
           id_student INTEGER, 
           studied_credits INTEGER,
           final_result TEXT);")

dbGetQuery(mydb, "SELECT * FROM sample1;")
dbGetQuery(mydb, "SELECT * FROM sample2;")

dbGetQuery(mydb,"SELECT l.id_student AS sample1, r.id_student AS sample2 
           FROM sample1 AS l 
           JOIN sample2 AS r ON l.id_student = r.id_student")

dbGetQuery(mydb,"SELECT l.id_student AS sample1, r.id_student AS sample2 
           FROM sample1 AS l 
           RIGHT JOIN sample2 AS r ON l.id_student = r.id_student")

dbGetQuery(mydb,"SELECT l.id_student AS sample1, r.id_student AS sample2 
           FROM sample1 AS l 
           LEFT JOIN sample2 AS r ON l.id_student = r.id_student")

#In here, are three different methods to join thoes tables? or it's whole process. 
