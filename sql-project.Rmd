...
db_user <- 'admin'
db_password <- 'Gx201314!'
db_name <- 'oudb'
db_host <- 'database-1.cs1l2ujaqoo0.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
library(dplyr)
toydata1 <- select(studentInfo,"id_student","gender","region","highest_education","imd_band")
toydata2 <- select(studentAssessment,"id_assessment","id_student","date_submitted","is_banked")
...
```{r}
#Query a portion of the database (always returns dataframe)查询数据库的一部分(总是返回数据帧)
...
dbGetQuery(mydb,"SELECT region, gender, id_student AS 'ID_Student' FROM Toydata1 ORDER BY id_student DESC LIMIT 20;")

#Read the other table according to a condition of one of the variables.
dbGetQuery(mydb,"SELECT * FROM Toydata1 WHERE highest_education ='HE Qualification';")
...
#Update a row   ###重点！！ Update就是Change###
...
dbGetQuery(mydb,"INSERT INTO Toydata1(id_student, gender, region, imd_band) VALUE ('0013','M','North Region','90-100%');") 
dbGetQuery(mydb,"UPDATE Toydata1 SET gender ='F' WHERE id_student = 0013;")
dbGetQuery(mydb,"SELECT * FROM Toydata1 ORDER BY id_student LIMIT 15;")
dbGetQuery(mydb,"DELETE FROM Toydata1 WHERE id_student = 13;")
```
...
dbGetQuery(mydb,"CREATE TABLE Toydata_1(
           id_student INTEGER,
           gender TEXT,
           region TEXT,
...
dbGetQuery(mydb,"SELECT * FROM Toydata_1;")
dbGetQuery(mydb,"DROP TABLE Toydata_1")
```
...
dbGetQuery(mydb,"SELECT * FROM Toydata_1")
dbGetQuery(mydb,"INSERT INTO Toydata_1(
           id_student,
           gender,
           region,
           highest_education,
           imd_band)
           VALUE ('13','M','NULL','HE Qualification','NULL')
           ;")
dbGetQuery(mydb, "SELECT * FROM Toydata_1;")
dbGetQuery(mydb, "DROP TABLE Toydata_1;")
```
...
dbGetQuery(mydb,"ALTER TABLE Toydata1 ADD cellphone INTEGER DEFAULT 3")
dbGetQuery(mydb,"SELECT *FROM Toydata1 LIMIT 5;")
dbGetQuery(mydb,"ALTER TABLE Toydata1 DROP COLUMN cellphone;")
...
dbGetQuery(mydb,"SELECT id_student, gender, region FROM Toydata1 WHERE gender = 'M';")
dbGetQuery(mydb,"SELECT id_student, gender, region FROM Toydata1 WHERE region LIKE '__e%';")
```
...
dbGetQuery(mydb,"INSETER INTO Toydata1(
           id_student,
           gender,
           region,
           highest_education,
           imd_band)
           VALUE('11391','M','East Anglian Region','HE Qualification','90-100%')
           ;")
dbGetQuery(mydb,"SELECT DISTINCT region FROM Toydata1")
```
...
dbGetQuery(mydb,"SELECT l.id_student AS Toydata1, r.id_student AS Toydata2 
           FROM Toydata1 AS l 
           JOIN Toydata2 AS r ON l.id_student = r.id_student")
dbGetQuery(mydb,"SELECT l.id_student AS Toydata1, r.id_student AS Toydata2 
           FROM Toydata1 AS l 
           RIGHT JOIN Toydata2 AS r ON l.id_student = r.id_student")
dbGetQuery(mydb,"SELECT l.id_student AS Toydata1, r.id_student AS Toydata2 
           FROM Toydata1 AS l 
           LEFT JOIN Toydata2 AS r ON l.id_student = r.id_student")
