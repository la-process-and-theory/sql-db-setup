...
db_name <- 'oudb'
db_host <- 'database-1.cmkqrwz9summ.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
library(dplyr)
sample1<-select(studentInfo,"id_student","gender","region","imd_band")
sample2<-select(studentInfo,"id_student","studied_credits","final_result")
dbWriteTable(mydb, "student_Info",sample1)
dbWriteTable(mydb, "student_Result",sample2)
dbListTables(mydb)
dbReadTable(mydb, "student_Info")
dbReadTable(mydb, "student_Result")
```
...
dbGetQuery(mydb,"SELECT id_student AS 'Student_ID', gender, region FROM student_Info ORDER BY id_student DESC LIMIT 20;")
dbGetQuery(mydb, "SELECT COUNT(*) FROM student_Result WHERE final_result='Pass';")
...
#insert
dbGetQuery(mydb, "INSERT INTO student_Info (id_student, gender, region, imd_band) VALUES ('00001', 'F', 'North Western Region', 'NULL');")
dbGetQuery(mydb, "SELECT * FROM student_Info ORDER BY id_student LIMIT 10;")
#change
dbGetQuery(mydb, "UPDATE student_Result SET studied_credits = '90' WHERE id_student = 11391;")
dbGetQuery(mydb, "SELECT id_student, studied_credits FROM student_Result ORDER BY id_student LIMIT 10;")
#delete
dbGetQuery(mydb, "DELETE FROM student_Result WHERE id_student = 11391;")
dbGetQuery(mydb, "SELECT * FROM student_Result ORDER BY id_student LIMIT 10;")
dbGetQuery(mydb, "DELETE FROM student_Info WHERE id_student = 00001;")
dbGetQuery(mydb, "SELECT * FROM student_Info ORDER BY id_student LIMIT 10;")
```
...
dbGetQuery(mydb,"CREATE TABLE student_INFO (
  id_student INTEGER, 
  gender TEXT,
  region TEXT,
  imb_band TEXT);")
dbGetQuery(mydb,"SELECT * FROM student_INFO;")
dbGetQuery(mydb, "DROP TABLE student_INFO;")
...
#Recreate 
dbGetQuery(mydb,"CREATE TABLE student_INFO_recreate (
  id_student INTEGER DEFAULT 0, 
  gender TEXT,
  region TEXT,
  imb_band TEXT);")
#Test
dbGetQuery(mydb,"INSERT INTO student_INFO_recreate (id_student, gender,region,imb_band) VALUES ('99998', 'F','North East','70-80%');")
dbGetQuery(mydb,"INSERT INTO student_INFO_recreate (id_student, gender,region,imb_band) VALUES ('99997', 'M','North West','20-30%');")
dbGetQuery(mydb, "SELECT * FROM student_INFO_recreate;")
dbGetQuery(mydb, "DROP TABLE student_INFO_recreate;")
```
...
dbGetQuery(mydb, "ALTER TABLE student_Result ADD times INTEGER DEFAULT 3 ")
dbGetQuery(mydb, "SELECT * FROM student_Result LIMIT 10;")
dbGetQuery(mydb, "ALTER TABLE student_Result DROP COLUMN times;")
```
...
dbGetQuery(mydb,"CREATE TABLE sample3 (
  student_id INTEGER AUTO_INCREMENT PRIMARY KEY,
  name TEXT,
  score INTEGER, 
  grade TEXT);")
dbGetQuery(mydb,"INSERT INTO sample3 (name, score,grade) VALUES ('Amy', 93,'A');")
dbGetQuery(mydb,"INSERT INTO sample3 (name, score,grade) VALUES ('Bob', 84,'B');")
dbGetQuery(mydb, "SELECT * FROM sample3;")
dbGetQuery(mydb, "DROP TABLE IF EXISTS sample3;")
```
...
dbGetQuery(mydb, "SELECT id_student, gender, region FROM student_Info WHERE region LIKE '__o%';")
dbGetQuery(mydb, "SELECT id_student, gender, region FROM student_Info WHERE region IN ('Yorkshire Region');")
```
...
dbGetQuery(mydb,"INSERT INTO student_Info (id_student, gender, region, imd_band) VALUES (28400, 'F',	'Scotland',	'20-30%');")
dbGetQuery(mydb, "SELECT DISTINCT id_student, gender, region, imd_band FROM student_Info;")
```
...
dbGetQuery(mydb, "SELECT * FROM student_Info;")
dbGetQuery(mydb, "SELECT * FROM student_Result;")

dbGetQuery(mydb,"SELECT l.id_student AS student_Info, r.id_student AS student_Result 
           FROM student_Info AS l 
           JOIN student_Result AS r ON l.id_student = r.id_student")
dbGetQuery(mydb,"SELECT l.id_student AS student_Info, r.id_student AS student_Result  
           FROM student_Info AS l 
           RIGHT JOIN student_Result AS r ON l.id_student = r.id_student")
dbGetQuery(mydb,"SELECT l.id_student AS student_Info, r.id_student AS student_Result  
           FROM student_Info AS l 
           LEFT JOIN student_Result AS r ON l.id_student = r.id_student")
```
```{r}
