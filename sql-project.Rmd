...
db_password <- 'testsql!'
db_name <- 'oudb'
db_host <- 'database-1.cpj2zbhtmpr6.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
id <- seq(1, 50,1)
happy_score <- sample(1:10,50, replace = TRUE)
city <- sample((c('New York','Dallas','Los Angeles','Seattle')),50,replace = TRUE)

toy1 <- data.frame(id,happy_score,city)

id <- seq(1, 50,1)
sad_score <- sample(1:10,50, replace = TRUE)
city <- sample((c('New York','Dallas','Los Angeles','Seattle')),50,replace = TRUE)

toy2 <- data.frame(id,sad_score,city)
...
dbGetQuery(mydb, "SELECT id AS 'Student ID', happy_score AS 'Happy Score', city As 'City' FROM toy1 ORDER BY id DESC LIMIT 20;")

#Read the other table according to a condition of one of the variables.

dbGetQuery(mydb, "SELECT id AS 'Student ID', sad_score AS 'Sad Score', city As 'City' FROM toy2 WHERE city = 'New York';")
...
#View inserted row
dbGetQuery(mydb, "SELECT * FROM studentAssessment ORDER BY id_assessment LIMIT 10;")
...
dbGetQuery(mydb, "SELECT id_student, score FROM studentAssessment ORDER BY id_assessment LIMIT 10;")
...
dbGetQuery(mydb, "INSERT INTO toy1 (id, happy_score) VALUES ('55', '10');")

dbGetQuery(mydb, "UPDATE toy2 SET sad_score = '1' WHERE id = 10;")

dbGetQuery(mydb, "SELECT id AS 'Student ID', happy_score AS 'Happy Score', city As 'City' FROM toy1 ORDER BY id;")
dbGetQuery(mydb, "SELECT id AS 'Student ID', sad_score AS 'Sad Score', city As 'City' FROM toy2 ORDER BY id;")

dbGetQuery(mydb, "DELETE FROM toy1 WHERE id = 55;")
dbGetQuery(mydb, "DELETE FROM toy2 WHERE id = 10;")
...
dbGetQuery(mydb,"CREATE TABLE toy_test (
  id INTEGER, 
  happy_score INTEGER,
  city TEXT
  );")

dbGetQuery(mydb, "INSERT INTO toy_test VALUES ( 1, 10, 'New York' );")
dbGetQuery(mydb, "INSERT INTO toy_test VALUES ( 2, 2, 'Los Angeles' );")
dbGetQuery(mydb, "INSERT INTO toy_test VALUES ( 3, 6, 'Dallas' );")

dbListTables(mydb)

dbGetQuery(mydb, "SELECT * FROM toy_test;")

dbGetQuery(mydb, "DROP TABLE toy_test;")
...
dbGetQuery(mydb,"CREATE TABLE toy_test (
   id INTEGER, 
   happy_score INTEGER DEFAULT 0,
   city TEXT
   );")

dbGetQuery(mydb, "INSERT INTO toy_test (id, city) VALUES ( 1, 'New York' );")
dbGetQuery(mydb, "INSERT INTO toy_test (id, city) VALUES ( 2, 'Los Angeles' );")
dbGetQuery(mydb, "INSERT INTO toy_test (id, city) VALUES ( 3, 'Dallas' );")

dbGetQuery(mydb, "SELECT * FROM toy_test;")

dbGetQuery(mydb, "DROP TABLE IF EXISTS toy_test;")
...
dbGetQuery(mydb, "ALTER TABLE toy1 ADD weight INTEGER DEFAULT 3 ")

dbGetQuery(mydb, "SELECT * FROM toy1 LIMIT 10;")

dbGetQuery(mydb, "ALTER TABLE toy1 DROP COLUMN weight;")
...
dbGetQuery(mydb,"INSERT INTO toy3 (happy_score, city, weight) VALUES (5, 'New York', 100);")
dbGetQuery(mydb,"INSERT INTO toy3 (happy_score, city, weight) VALUES (10, 'Dallas', 120);")
...
dbGetQuery(mydb, "SELECT * FROM toy1 WHERE city = 'New York' AND happy_score > 8;")
...
dbGetQuery(mydb, "INSERT INTO toy1 (id, happy_score, city) VALUES ( 41, 10, 'New York' );")

dbGetQuery(mydb, "SELECT DISTINCT id, happy_score, city FROM toy1;")
...
dbGetQuery(mydb,"SELECT toy1.happy_score, toy2.sad_score 
           FROM toy1 
           LEFT JOIN toy2 ON toy1.id = toy2.id")
