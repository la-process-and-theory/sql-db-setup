...
db_password <- 'testsql!'
db_name <- 'oudb'
db_host <- 'database-1.cuypbaz2virv.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
df1 = data.frame(number1 = 1:30, number2 = 31:60, character1 = rep("a",30))
df2 = data.frame(number1 = 61:90, number2 = 31:60, character1 = rep("b",30))
dbWriteTable(mydb, "df1", df1)
dbWriteTable(mydb, "df2", df2)
dbListTables(mydb)
```
...
dbGetQuery(mydb, "SELECT number1 AS number3, number2, character1 FROM df1 ORDER BY number2 DESC LIMIT 20;")

#Read the other table according to a condition of one of the variables.
dbGetQuery(mydb, "SELECT * FROM df2 WHERE number2 > 40;")
```
...
dbGetQuery(mydb, "INSERT INTO df1 (number1, character1) VALUES (4, 'v');")
dbGetQuery(mydb, "UPDATE df2 SET number2 = 12 WHERE number2 = 45;")


dbGetQuery(mydb, "SELECT * FROM df1;")
dbGetQuery(mydb, "SELECT * FROM df2;")
dbGetQuery(mydb, "DELETE FROM df1 WHERE character1 = 'v';")
dbGetQuery(mydb, "DELETE FROM df2 WHERE number2 = 12;")
```
...
dbGetQuery(mydb,"CREATE TABLE df3 (
  number1 INTEGER, 
  number2 INTEGER,
  character1 TEXT
  );")
dbGetQuery(mydb,"INSERT INTO df3(number1, number2, character1) SELECT number1, number2, character1 FROM df1;")
dbGetQuery(mydb,"SELECT * FROM df3;")
dbGetQuery(mydb, "DROP TABLE df1;")
```
...
dbGetQuery(mydb,"CREATE TABLE df4 (
  number1 INTEGER DEFAULT 0, 
  number2 INTEGER DEFAULT 0,
  character1 TEXT
  );")
dbGetQuery(mydb,"INSERT INTO df4 (character1) VALUES ('B');")
dbGetQuery(mydb,"SELECT * FROM df4;")
dbGetQuery(mydb, "DROP TABLE IF EXISTS df4;")
```
...
dbGetQuery(mydb, "ALTER TABLE df3 ADD number3 INTEGER DEFAULT 3 ")

dbGetQuery(mydb, "SELECT * FROM df3;")
dbGetQuery(mydb, "ALTER TABLE df3 DROP COLUMN number3;")
```
...
dbGetQuery(mydb,"CREATE TABLE df5 (
...
dbGetQuery(mydb,"INSERT INTO df5 (number1, character1, number2, number3) VALUES (1, 'A', 2, 3);")
dbGetQuery(mydb,"SELECT * FROM df5;")
```
...
dbGetQuery(mydb, "SELECT number1, number2 FROM df3 WHERE number1 > 3 AND number2 < 40 ORDER BY number1 DESC;")
```
...
dbGetQuery(mydb,"INSERT INTO df3 (row_names, number1, character1, number2) VALUES ('1', 1, 'a', 31);")
dbGetQuery(mydb, "SELECT * FROM df3;")
dbGetQuery(mydb, "SELECT DISTINCT * FROM df3;")
```
...
dbGetQuery(mydb,"SELECT l.number1 AS df3, r.number1 AS df2
           FROM df3 AS l
           JOIN df2 AS r ON l.number2 = r.number2")
```
