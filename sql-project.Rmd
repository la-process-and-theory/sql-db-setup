...
db_password <- 'testsql!'
db_name <- 'oudb'
db_host <- 'database-1.cdiu1nscocjd.us-east-2.rds.amazonaws.com'
db_port <- 3306
...
library(dplyr)
toydata1<-select(studentInfo,code_module,code_presentation,id_student,gender,num_of_prev_attempts,studied_credits,disability,final_result)
toydata2<-select(studentInfo,id_student,gender,region,highest_education,imd_band,age_band)
toydata1<-toydata1[c(1:30),]
toydata2<-toydata2[c(1:30),]
dbWriteTable(mydb,"stuinfo1",toydata1)
dbWriteTable(mydb,"stuinfo2",toydata2)
dbListTables(mydb)
...
dbGetQuery(mydb, "SELECT code_module, code_presentation, id_student AS 'Student ID', gender, num_of_prev_attempts, studied_credits, disability, final_result FROM stuinfo1 ORDER BY 'Student ID' DESC LIMIT 20;") 


#Read the other table according to a condition of one of the variables.
dbGetQuery(mydb, "SELECT * FROM stuinfo2 WHERE region = 'Ireland';")
...
dbGetQuery(mydb, "INSERT INTO stuinfo2 (id_student, gender, region, highest_education, age_band) VALUES ('1', 'F', 'Wales', 'A level or Equivalent', '0-35');")
dbGetQuery(mydb, "SELECT * FROM stuinfo2 ORDER BY id_student LIMIT 10;")

dbGetQuery(mydb, "UPDATE stuinfo1 SET studied_credits = '120' WHERE id_student = 11391;")
dbGetQuery(mydb, "SELECT * FROM stuinfo1 ORDER BY id_student LIMIT 10;")

dbGetQuery(mydb, "DELETE FROM stuinfo2 WHERE id_student = 1;")
dbGetQuery(mydb, "SELECT * FROM stuinfo2 ORDER BY id_student;")

dbGetQuery(mydb, "DELETE FROM stuinfo1 WHERE id_student = 11391;")
dbGetQuery(mydb, "SELECT * FROM stuinfo1 ORDER BY id_student;")
```
...
dbGetQuery(mydb,"CREATE TABLE stuinfo1_1 (code_module TEXT, code_presentation TEXT, id_student INTEGER, gender TEXT, num_of_prev_attempts INTEGER, studied_credits INTEGER, disability TEXT, final_result TEXT);")
dbListTables(mydb)
#dbGetQuery(mydb, "DROP TABLE stuinfo1;")
#dbListTables(mydb)
```
...
dbGetQuery(mydb,"
                INSERT INTO test_stuinfo2(
                          ...
                          age_band)
                VALUES
                ('M','Ireland',NULL ,'30-40%','0-35'),
                ('F','Ireland','Lower Than A Level', NULL, '0-35')
                ")
dbGetQuery(mydb, "SELECT * FROM test_stuinfo2;")
dbGetQuery(mydb,"DROP TABLE test_stuinfo2;")
dbListTables(mydb)
...
dbGetQuery(mydb, "ALTER TABLE stuinfo2 ADD score INTEGER DEFAULT 3 ")

dbGetQuery(mydb, "SELECT * FROM stuinfo2 LIMIT 10;")

dbGetQuery(mydb, "ALTER TABLE stuinfo2 DROP COLUMN score;")
...
dbGetQuery(mydb,"
           ...
           ")
dbListTables(mydb)
dbGetQuery(mydb," INSERT INTO table0 (gender,time,age) 
                  VALUES('F','2021.1.25','25'),
                        ('M','2020.11.7','21')")
dbGetQuery(mydb,"SELECT * FROM table0;")
```
...
dbGetQuery(mydb, "SELECT id_student, gender, highest_education FROM stuinfo2 WHERE gender ='F' AND highest_education = 'A Level or Equivalent' ORDER BY id_student DESC;")

dbGetQuery(mydb, "SELECT id_student, gender, region FROM stuinfo2 WHERE gender ='M' and region like '%East%';")
```
...
dbGetQuery(mydb,"SELECT * FROM stuinfo2 ORDER BY id_student;")
dbGetQuery(mydb,"INSERT INTO stuinfo2 VALUES  (2,28400,'F','Scotland','HE Qualification','20-30%','35-55');")
dbGetQuery(mydb,"SELECT * FROM stuinfo2 ORDER BY id_student;")

dbGetQuery(mydb, "SELECT DISTINCT * FROM stuinfo2;")
```
...
dbGetQuery(mydb, "SELECT DISTINCT * FROM stuinfo1;")
dbGetQuery(mydb, "SELECT DISTINCT * FROM stuinfo2;")

dbGetQuery(mydb, "INSERT INTO stuinfo1 (id_student) VALUES ( 1 );")
dbGetQuery(mydb, "INSERT INTO stuinfo1 (id_student) VALUES ( 2 );")
dbGetQuery(mydb, "INSERT INTO stuinfo2 (id_student) VALUES ( 1 );")
dbGetQuery(mydb, "INSERT INTO stuinfo2 (id_student) VALUES ( 2 );")

dbGetQuery(mydb,"SELECT distinct l.*, r.*
           FROM stuinfo1 l 
           JOIN stuinfo2 r 
           ON l.id_student = r.id_student")
```
```{r}
